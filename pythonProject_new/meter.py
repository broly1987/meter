...
def meter_pro():
        ...
        sg.theme('DarkTeal10')
        layout = [
            [sg.Text("Lockname:"), sg.Input(key="-LOCKNAME-", size=(17, 1),background_color="White")],
            [sg.Text("Path:"), sg.Combo(rutas_archivos, key="-RUTA-", size=(50, 1),background_color="White")],
            [sg.Text("Meter:"), sg.Combo(["total in", "total out"], key="-METER-", size=(19, 1),background_color="White")],
            [sg.Text("Time:"), sg.Input(key="-TIME-", size=(17, 1),background_color="White")],
            [sg.Button("Search")],
            [sg.Text("total_in_mach:"), sg.Input(key="-ROLL-", size=(17, 1),background_color="White")],
            [sg.Text("incrementation:"), sg.Input(key="-INC-", size=(17, 1),background_color="White")],
            [sg.Text("Result:"), sg.Text("",key="-RESULT-", size=(30, 1),text_color="White")],
            [sg.Button("Calculate")],
            [sg.Text("Manager:"), sg.Combo(["Ariel", "Ernest"], key="-SUPERVISOR-", size=(18, 1),background_color="White")],
            [sg.Button("Save")],
            [sg.Button("Check for Updates...")],
            [sg.ProgressBar(100, orientation='h', size=(20, 20), key='-PROGRESS-', visible=True)]
        ]

        ventana = sg.Window("Roll Over EGM", layout,finalize=True)
        ...
        while True:
            ...
            if evento == sg.WINDOW_CLOSED:
                ...
                resultado = None
                if not  lockname:
                    sg.popup("please enter a lockname ")

                elif len(lockname) !=5:
                    sg.popup("must be 5 digits ")
                ...
                if hoja and hoja['E1'].value is None:
                    hoja['E1'] = 'Manager'
                if hoja and hoja['F1'].value is None:
                    hoja['F1'] = 'total_in_machine'
                if hoja and hoja['G1'].value is None:
                    hoja['G1'] = 'Incrementation'
                if hoja and hoja['H1'].value is None:
                    hoja['H1'] = 'Total'
